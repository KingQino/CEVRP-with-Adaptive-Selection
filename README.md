...
- Online operator selection learns only during the post-probe continuation. For each operator it discounts and accumulates the original V1.1 downstream credit and distance-call cost relative to the individual's weak probe, then selects from the active RVND operators using robustly standardized `log1p(reward / cost)` with uncertainty and 5% uniform exploration. Its observation count is the number of individuals on which the operator was called, not the raw call count.

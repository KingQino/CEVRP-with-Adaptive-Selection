...
- Evolution rows are sampled every 10 generations. Local-search operator, allocation, and operator-learning diagnostics are aggregated into 50-generation windows, with a final partial snapshot written at termination. Operator cost combines distance calls with lightweight route, route-pair, and candidate-scan work units; both components are normalized against the common weak probe and do not alter the global distance-call budget.

...
- Online operator selection retains the V1.1 reward, cost, discounting, and exploration model, but keeps independent policies for medium and deep continuation. The compact operator-learning log includes the continuation context and its effective observation count so the two policies can be analyzed separately.

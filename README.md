...
- Allocated policies preserve each individual's RVND session and failure cache while progressing from weak to medium or strong. The intensity reward combines lower-archive rank, post-probe gamma crossing, and reproduction parent usage. Continuation upper gain remains an aggregate diagnostic signal but is excluded from the intensity reward. Action cost contains only evaluations consumed after the common weak probe.
- Online operator selection learns only during the post-probe continuation. Its reward preserves the downstream parent, lower-archive, and gamma credits while adding a bounded continuation upper-gain signal with weight `0.05`; this immediate signal affects only operator credit and does not change the intensity learner's Reward V3.1.

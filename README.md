...
| `-op_policy` | `uniform` | Continuation operator-selection policy: `uniform` or `online` |
| `-op_cost_weight` | `1.0` | Weight of the standardized operator distance-call cost |
...
- Online operator learning retains V1.1 reward attribution and discounts reward and cost separately. Cost V2 accumulates raw distance calls, averages them per operator call, and applies `-op_cost_weight` after robust cost standardization instead of normalizing every record by its weak-probe cost.
